...
- Token 数量旁附带成本分项
- Claude Code、Codex 与 Cursor 的用量上限检测，涵盖 session、每周、账单与 credits 窗口
- 外观控制：玻璃透明度、模糊度、完全透明窗口
- 菜单栏（macOS）与系统托盘（Windows）弹出窗口，图标旁可显示成本、token 数，或 Claude／Codex／Cursor 最接近用完的用量上限百分比
- 本地优先：单设备使用完全无需服务器
...
- **AI 工具用量上限**——选择 Claude Code、Codex 与 Cursor 的用量上限检测与刷新频率。
- **显示模式**——可将浮动窗口改为 macOS 菜单栏或 Windows 系统托盘的弹出窗口，并选择图标旁显示的内容：成本、今日 token 数、累计 token 数、成本＋token、最接近用完的 Claude／Codex／Cursor 用量上限百分比，或仅显示图标。
...
TOKEN_MONITOR_LIMIT_PROVIDERS=       # 可选——默认为所有支持的提供方（claude、codex、cursor）
```
...
- 按客户端与模型的分项统计
- 启用 AI 工具用量上限时，归一化后的 Claude Code／Codex／Cursor 用量状态
